...
export const fetchTokenIconData = async (symbol: string) => {
    ...
    return await axios.get(`${TOKEN_API_URL}/tokens/${assetKey}/manifest.json`)
        .then((res) => res.data as TokenManifest)
        .then((response) =>
            getImageBase64Data(
                `${TOKEN_API_URL}${response.svgIconPath}`
            ).catch(() => undefined)
        ).catch(() => DEFAULT_TOKEN);
};

export const fetchTokenData = async (token: string, chainId: string) => axios.post(`https://api.thegraph.com/subgraphs/name/superfluid-finance/${networks[Number(chainId)].subgraphId}`, {
